...
#pragma omp parallel for schedule(dynamic, 1)
        for (auto id = 0; id < num_points; ++id)
        {
            ...
            // 2.1 搜索距离最近的点 search for point
            const char *query = _base_storage->get_vector(id);     // 获取当前节点的向量数据
            iterate_to_fixed_point(query, search_cache, true, id); // 在当前节点的 Vamana 图索引中搜索最近邻节点

            // 2.2 修建候选邻居列表 prune for candidate neighbors
            std::vector<IdxType> pruned_list;
            prune_neighbors(id, search_cache->expanded_list, pruned_list, search_cache);

            // 2.3 update neighbors and insert the reversed edge
            {
                std::lock_guard<std::mutex> lock(_graph->neighbor_locks[id]);
                _graph->neighbors[id] = pruned_list;
            }
            inter_insert(id, pruned_list, search_cache); // 为每个邻居插入反向边，确保图的连通性
        ...
        // 3. 最终修剪邻居列表，保证每个点的邻居数量不超过指定值
#pragma omp parallel for schedule(dynamic, 1)
        for (auto id = 0; id < num_points; ++id)
            if (_graph->neighbors[id].size() > _max_degree) // 当前节点的邻居数量超过 _max_degree
            {

                // 3.1 计算当前节点与邻居节点之间的距离 prepare candidates
                                 ...
                                 std::shared_ptr<SearchCache> search_cache)
    {
        ...
        // 1. init candidates，按照距离从小到大排序候选邻居列表
        std::sort(candidates.begin(), candidates.end(), [](const Candidate &a, const Candidate &b)
                  { return a.distance < b.distance; });
        auto candidate_size = std::min((IdxType)(candidates.size()), _max_candidate_size);

        // 2. 初始化遮挡因子 init occlude factor
        auto &occlude_factor = search_cache->occlude_factor;
        occlude_factor.clear();
        occlude_factor.insert(occlude_factor.end(), candidate_size, 0.0f);

        // 3. 修剪邻居prune neighbors
        float cur_alpha = 1;
        while (cur_alpha <= _alpha && pruned_list.size() < _max_degree)
        {
            for (auto i = 0; i < candidate_size && pruned_list.size() < _max_degree; ++i) // 遍历候选邻居列表
            {
                // 如果当前候选邻居的遮挡因子大于 cur_alpha，则跳过
                if (occlude_factor[i] > cur_alpha)
                    continue;

                occlude_factor[i] = std::numeric_limits<float>::max(); // 将当前候选邻居的遮挡因子设为最大值，避免重复考虑
                if (candidates[i].id != id)
                    pruned_list.push_back(candidates[i].id); // 将其添加到修剪后的邻居列表中
