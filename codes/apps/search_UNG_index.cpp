...
// 处理单个查询
void process_query_cost(int query_id, int K, std::pair<ANNS::IdxType, float> *gt, std::mutex &mtx, std::string cost_file, std::string sort_cost_file)
{
...
void save_sort_cost_files(std::pair<ANNS::IdxType, float> *gt, int num_queries, int K, std::string cost_file, std::string sort_cost_file)
{
   ...
   {
      threads.emplace_back(process_query_cost, query_id, K, gt, std::ref(mtx), cost_file, sort_cost_file);
   }
...
// ============================================ 存储dist文件=================================================
auto comp_dis = [](const DistEntry &a, const DistEntry &b)
{
   return a.dist < b.dist; // 最大堆
};
// 读取 nearest_nei_dist/0_nearest_nei_dist.csv 文件
void read_dist_file(const std::string &file_path, std::vector<DistEntry> &dist_entries)
{
   std::ifstream file(file_path);
   if (!file.is_open())
   {
   ...
   while (std::getline(file, line))
   {
      ...
      // 读取 Cost
      std::getline(ss, value, ',');
      entry.dist = std::stof(value);

      dist_entries.push_back(entry);
   }
   file.close();
}

// 将排序后的 gt 结果存入文件
void save_sorted_dist(const std::vector<DistEntry> &sorted_entries, int query_id, std::string sort_dist_file)
{
   std::string output_dir = sort_dist_file;
   fssy::create_directories(output_dir); // 确保目录存在

   std::string output_file = output_dir + "sorted_dist_result_" + std::to_string(query_id) + ".csv";
   std::ofstream file(output_file);
   ...
   for (const auto &entry : sorted_entries)
   {
      file << entry.vector_id << "," << entry.dist << "\n";
   }
...
// 处理单个查询
void process_query_dist(int query_id, int K, std::pair<ANNS::IdxType, float> *gt, std::mutex &mtx, std::string dist_file, std::string sort_dist_file)
{
   std::string dist_dir = dist_file;
   std::string file_path = dist_dir + std::to_string(query_id) + "_nearest_nei_dis" + ".csv";
   std::vector<DistEntry> dist_entries;

   if (fssy::exists(file_path))
   {
      read_dist_file(file_path, dist_entries);
   }
   else
   {
      std::cerr << "File not found: " << file_path << std::endl;
   ...
   // 使用优先队列找到最小的 K*2 个
   std::priority_queue<DistEntry, std::vector<DistEntry>, decltype(comp_dis)> pq(comp_dis);

   for (const auto &entry : dist_entries)
   {
      pq.push(entry);
      if (pq.size() > K * 2)
      {
   ...
   // 将结果存入 gt
   std::vector<DistEntry> top_k_entries;
   while (!pq.empty())
   {
      top_k_entries.push_back(pq.top());
      pq.pop();
   ...
   for (int i = 0; i < top_k_entries.size(); ++i)
   {
      gt[query_id * K + i] = std::make_pair(top_k_entries[i].vector_id, top_k_entries[i].dist);
   }

   // 将排序后的结果存入 sort_dist 目录
   save_sorted_dist(top_k_entries, query_id, sort_dist_file);
}

// 加载所有 dist 文件并找到最小的 K 个(并行)
void save_sort_dist_files(std::pair<ANNS::IdxType, float> *gt, int num_queries, int K, std::string dist_file, std::string sort_dist_file)
{
   std::vector<std::thread> threads;
   std::mutex mtx;
   std::cout << "enter save_sort_dist_files" << std::endl;
   for (int query_id = 0; query_id < num_queries; ++query_id)
   {
      threads.emplace_back(process_query_dist, query_id, K, gt, std::ref(mtx), dist_file, sort_dist_file);
   }
...
// 从已排序的 dist 文件中读取并将结果存入 gt
void load_sorted_dist_to_gt(std::pair<ANNS::IdxType, float> *gt, int num_queries, int K, std::string sort_dist_file)
{
   std::string sorted_dist_dir = sort_dist_file;

   for (int query_id = 0; query_id < num_queries; ++query_id)
   {
      // std::cout << "Processing query_id: " << query_id << std::endl;
      std::string file_path = sorted_dist_dir + "sorted_dist_result_" + std::to_string(query_id) + ".csv";
      std::ifstream file(file_path);
      ...
      while (std::getline(file, line) && i < K)
      {
         ...
         // 将 entry 添加到 dist_entries
         dist_entries.push_back(entry);

         // 将结果存入 gt
         gt[query_id * K + i] = std::make_pair(entry.vector_id, entry.dist);
...
int main(int argc, char **argv)
{
   ...
   std::string cost_file, sort_cost_file, dist_file, sort_dist_file;
   ANNS::IdxType K, num_entry_points;
   std::vector<ANNS::IdxType> Lsearch_list;
   uint32_t num_threads;
   int gt_mode;
   bool cal_use_dist_or_cost = false; // false:dist  true:cost
   ...
   try
   {
      ...
      desc.add_options()("dist_file", po::value<std::string>(&dist_file)->required(),
                         "Filename for the dist_file");
      desc.add_options()("sort_dist_file", po::value<std::string>(&sort_dist_file)->required(),
                         "Filename for the sort_dist_file");
   ...
   query_storage->load_from_file(query_bin_file, query_label_file_r, 1000);

   std::shared_ptr<ANNS::IStorage> query_storage_o = ANNS::create_storage(data_type);
   query_storage_o->load_from_file(query_bin_file, query_label_file_o, 1000);
   ...
   std::cout << "gt_mode = " << gt_mode << std::endl;
   if (gt_mode == 0) // 有自带的gt.bin文件
      ANNS::load_gt_file(gt_file, gt, num_queries, K);
   else if (gt_mode == 1) // 计算最近dist，并存储到文件夹中
      save_sort_dist_files(gt, num_queries, K, dist_file, sort_dist_file);
   else if (gt_mode == 2) // 从sort_dist文件夹中读取
      load_sorted_dist_to_gt(gt, num_queries, K, sort_dist_file);
   else if (gt_mode == 3) // 计算最近cost，并存储到文件夹中
      save_sort_cost_files(gt, num_queries, K, cost_file, sort_cost_file);
   else if (gt_mode == 4) // 从sort_cost文件夹中读取
   ...
   for (auto Lsearch : Lsearch_list)
   {
      ...
      index.search(query_storage, query_storage_o, distance_handler, num_threads, Lsearch, num_entry_points, scenario, K, results, num_cmps, 0.5, cal_use_dist_or_cost);
